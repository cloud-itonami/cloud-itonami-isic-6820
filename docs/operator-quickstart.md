...
- `test/realty/phase_test.cljk` — Phase 0→3 invariants; `:fee/pay` and `:contract/execute` never auto-eligible at any phase
- `test/realty/registry_test.cljk` — Fee-payment/contract-execution draft record shape and conformance
...
| File | Role |
|---|---|
| `src/realty/store.cljk` | Store protocol (MemStore, DatomicStore); append-only audit ledger; no separate party concept |
| `src/realty/registry.cljk` | Fee and contract draft records; simplified fixed-percentage-of-rent formula |
| `src/realty/facts.cljk` | Jurisdiction-specific property-management disclosure/trust-account catalog with official spec-basis citations |
| `src/realty/realtyfeellm.cljk` | Realty-Fee-LLM Advisor (mock or real); intake/assessment/filing/payment/contract proposals |
| `src/realty/governor.cljk` | Independent verification layer; 7 HARD checks + double-payment guard |
| `src/realty/phase.cljk` | Phase table (0→3): read-only → assisted intake → assisted assess → supervised |
| `src/realty/operation.cljk` | OperationActor (langgraph-clj StateGraph) |
| `src/realty/sim.cljk` | Demo driver |

## Jurisdiction coverage

Current catalog (`src/realty/facts.cljk`): JPN, USA-NY, GBR, DEU (4/~194 worldwide). All entries cite official sources; never fabricated.

Adding a jurisdiction:
1. Add one map entry to `realty.facts/catalog` with official spec-basis reference
2. Add test case to `test/realty/facts_test.cljk`
