(ns realty.operation
  "OperationActor -- one real-estate-fee-services operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (Realty-Fee-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Real-Estate Fee-Services
  ...
  One graph run = one real-estate-fee-services operation (intake ->
  ...
  decision to a human operator (the licensed property manager). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:fee/pay`/`:contract/execute` ALWAYS reach this node
  when the governor is clean -- see `realty.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [realty.realtyfeellm :as realtyfeellm]
...
(defn build
  ...
  `realty.store/Store`).
  opts:
    :advisor      -- a `realty.realtyfeellm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (realtyfeellm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Realty-Fee-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (realtyfeellm/-advise advisor store request)]
            {:proposal p :audit [(realtyfeellm/trace request p)]})))

      ;; Real-Estate Fee-Services Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
